...
const Transfer: React.FC = () => {
  ...
  const setItemforSale = async (index: number) => {
    try {
      if (window.ethereum) {
        ...
        await contractInstance3.putItemForSale(tokenID);
  ...
  const handleTransfer = async (index: number) => {
    try {
      const newAddress = newAddresses[index];
      ...
      const contractInstance3 = new ethers.Contract(contractAddress2, abi2, signer);
      console.log("Get contractinstance");
      const price = newPrice[index];
      await contractInstance3.transfer(ownedItems[index], newAddress, price);
      console.log("sucess");

      // Optionally, update the state or perform any other actions after successful transfer
      ...
      <div>
        ...
        <div>
          {itemsInformation.length > 0 ? (
            <div>
              <h2>Fill in new owner and price and press transfer</h2>
                ...
                {itemsInformation.map((itemInformation, index) => (
                  <Card size="small" title={itemInformation[0].toString()} className="cardContainer">
                    ...
                    <div className="buttonContaier">
                      <Button type="primary" onClick={() => setItemforSale(index)}>
                        Set item for sale
                      </Button>
                      <Button type="primary" onClick={() => handleTransfer(index)}>
                        Transfer
                      </Button>
                    </div>
                  </Card>
